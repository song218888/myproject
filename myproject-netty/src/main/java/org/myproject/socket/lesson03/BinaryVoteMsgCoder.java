 ...
 * 
 * 
 * 下面将展示基于二进制格式对消息进行编码的程序。
 * 
 * 与基于文本的格式相反，二进制格式使用固定大小的消息，
 * 
 * 每条消息由一个特殊字节开始，该字节的最高六位为一个”魔术值“010101，
 * 
 * 该字节的最低两位对两个布尔值进行了编码，消息的第二个字节总是0，第三、四个字节包含了candidateID值，只有响应消息的最后8个字节才包含了选票总数信息。
 * 
 * 字节序列格式如下图所示：  
 * 
 * 参见：https://blog.csdn.net/ns_code/article/details/14229253
 * 
 * @author DELL
 *
 */
public class BinaryVoteMsgCoder implements VoteMsgCoder {
